 ...
 *
 * `/voice-flush` 완전 폐지(VoiceFlushCommand 삭제) 이후 모듈 메타데이터가
 * 의도한 6개 커맨드만 정확히 등록하고 있는지, 그리고 삭제된 커맨드가
 * 다시 섞여 들어오지 않는지(회귀 방지)를 검증한다.
 *
...
describe('BotCommandModule', () => {
  describe('providers', () => {
    it('voice-flush 폐지 후 정확히 6개의 커맨드만 등록되어 있다', () => {
